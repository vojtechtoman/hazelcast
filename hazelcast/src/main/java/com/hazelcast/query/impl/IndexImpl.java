...
public class IndexImpl extends AbstractIndex {
    ...
    public IndexImpl(String name, String[] components, boolean ordered, boolean fulltext, InternalSerializationService ss, Extractors extractors,
                     IndexCopyBehavior copyBehavior, PerIndexStats stats) {
        super(name, components, ordered, fulltext, ss, extractors, copyBehavior, stats);
    }

    @Override
    protected IndexStore createIndexStore(boolean ordered, boolean fulltext, PerIndexStats stats) {
        if (fulltext) {
            return new FulltextIndexStore(copyBehavior);
        }
